...
namespace App.Services
{
    ...
    {
        public static async Task<byte[]> ExportToExcelAsync(List<StaffResponseModel> staffList)
        {
            try
            {
                // Validate input
                if (staffList == null)
                {
                    throw new ArgumentNullException(nameof(staffList), "Staff list cannot be null.");
                }
                ...
                using (var range = worksheet.Cells[1, 1, 1, 7])
                {
                    ...
                    range.Style.Fill.BackgroundColor.SetColor(Color.LightGray); // Use System.Drawing.Color
                }

                // Data
                for (int i = 0; i < staffList.Count; i++)
                {
                    var staff = staffList[i] ?? throw new ArgumentException($"Staff item at index {i} is null.");
                    var row = i + 2;
            ...
            {
                // Log the exception (e.g., using a logging framework like Serilog or Microsoft.Extensions.Logging)
                throw new InvalidOperationException("Failed to export staff list to Excel.", ex);

            }
        }

        public static async Task<byte[]> ExportToPdfAsync(List<StaffResponseModel> staffList)
        {
